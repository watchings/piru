...
2. **默认本地优先、可选同步**：首个跨平台版本不强制引入账号或云端；先支持本地数据库、加密备份和显式导入导出。需要多设备同步时，再增加端到端加密同步服务。
    ...
    现有实现逐步适配             原生平台能力                    PWA/桌面浏览器
             ...
             └──────────────┬───────────┴──────────────┬───────────┘
                   ...
                   相同计算结果与 fixtures       相同物质库发布产物
                            │                          │
                 可选的端到端加密同步服务       Python 数据构建 pipeline
```
...
- 默认关闭，不影响离线使用。
- 服务端只保存不可读的加密信封；客户端负责密钥派生、加密、解密、冲突合并和导出。
- 第一版同步只支持 journal/设置/自定义覆盖层，不能修改或上传官方物质库。
...
- 保留现有 PsyLog-compatible JSON 导出作为兼容输入，但新增 Piru Native v2 规范。
...
- [ ] 确认是否坚持“无账号/无服务器”为默认产品承诺。
- [ ] 确认 Web 是 PWA、桌面封装应用，还是两者都要。
- [ ] 确认 Android 最低版本、发布渠道、是否需要 F-Droid/Google Play。
- [ ] 盘点所有现有实体、功能、权限、通知和平台依赖。
- [ ] 建立功能矩阵：iOS、macOS、Android、Web、不可支持/降级。
- [ ] 记录当前版本标签、SQLite manifest、导出样本和关键计算结果，形成迁移基线。

### 阶段 1：契约与测试基座

- [ ] 冻结实体、枚举、单位、时间和错误码。
- [ ] 编写 JSON Schema、示例数据、迁移规则和字段敏感性分类。
- [ ] 从现有模型生成/手写互操作 fixture，不直接复制 SwiftData schema。
- [ ] 为 PK/PD、单位换算、时间线、交互、耐受和聚类建立 golden fixtures。
- [ ] 建立三端一致性测试命令、版本矩阵和失败诊断格式。

### 阶段 2：物质库与只读参考

- [ ] 将现有 SQLite manifest/checksum 下载和校验逻辑抽象为平台无关协议。
- [ ] 实现 Web 的只读 catalog 导入、缓存、全文/前缀/模糊搜索和来源展示。
- [ ] 实现 Android 的只读 catalog 导入、索引和来源展示。
- [ ] 对 1,700+ 条目抽样校验名称、别名、剂量、时长、半衰期、来源优先级和引用。
- [ ] 验证 curated overlay、区域名称、中文内容和官方数据库更新不会产生平台分叉。

### 阶段 3：日志垂直切片

- [ ] Web：本地 journal、物质选择、快速记录、会话详情、编辑/撤销。
- [ ] Android：同一闭环，包含 Room migration 和进程被杀后的恢复。
- [ ] iOS：只增加契约适配和导入/导出兼容，不改变既有用户数据路径。
- [ ] 三端实现同一份导入、导出、重复记录和日期边界测试。

### 阶段 4：计算与时间线

- [ ] 在三端接入 PK/PD、active window、session clustering 和基础图表。
- [ ] 用 fixture 比较采样点、峰值、Tmax、消除曲线、零阶清除和时区转换。
- [ ] 交互结果按类别/规则/证据级别展示，并保留来源文本和“不是医疗建议”边界。
- [ ] 对极端剂量、空数据、缺失半衰期、跨午夜会话、夏令时和离线状态做测试。

### 阶段 5：备份、恢复与本地化

- [ ] 支持 Piru Native v2 加密备份的导出/导入。
...
- [ ] 仅在后端安全设计获批后实现端到端加密同步。
- [ ] 内测环境进行丢网、并发设备、时钟错误、冲突、撤销和恢复测试。
